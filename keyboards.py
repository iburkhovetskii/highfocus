...
def get_question_1_keyboard() -> ReplyKeyboardMarkup:
    ...
    options = [
        "А) Быстро собираюсь, делаю план и иду по нему. (🔒 Компульсивный)",
        "Б) Начинаю тревожиться и прокручивать негативные сценарии. (😰 Тревожный)",
        "В) Чувствую упадок и прокрастинирую. (😔 Депрессивный)",
        "Г) Заряжаюсь энергией, но могу вспылить. (🌋 Вспыльчивый)",
        "Д) Бросаюсь в задачу с азартом, но хаотично. (⚡ Импульсивный)",
        "Е) То включаюсь, то выгораю — скачки энергии. (🎭 Циклотимный)",
        "Ж) Спокойно адаптируюсь, распределяю силы. (☯ Сбалансированный)"
    ]
...
def get_question_2_keyboard() -> ReplyKeyboardMarkup:
    ...
    options = [
        "А) План и контроль. (🔒)",
        "Б) Уверенность, что всё под контролем. (😰)",
        "В) Поддержка и признание. (😔)",
        "Г) Азарт и драйв. (🌋)",
        "Д) Новизна и вызов. (⚡)",
        "Е) Вдохновение и увлечённость. (🎭)",
        "Ж) Баланс интереса и пользы. (☯)"
    ]
    random.shuffle(options)
    keyboard = [[KeyboardButton(text=option)] for option in options]
    return ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True, one_time_keyboard=True)
...
def get_question_3_keyboard() -> ReplyKeyboardMarkup:
    ...
    options = [
        "А) Всё раздражает, если что-то идёт не по плану. (🔒)",
        "Б) Не могу отключить тревожные мысли. (😰)",
        "В) Пустота и апатия. (😔)",
        "Г) Вспышки гнева по мелочам. (🌋)",
        "Д) Теряю интерес, ищу острые ощущения. (⚡)",
        "Е) Настроение скачет. (🎭)",
        "Ж) Постоянная усталость, даже после отдыха. (☯)"
    ]
    random.shuffle(options)
    keyboard = [[KeyboardButton(text=option)] for option in options]
    return ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True, one_time_keyboard=True)
...
def get_question_4_keyboard() -> ReplyKeyboardMarkup:
    ...
    options = [
        "А) Хаос и постоянные изменения. (🔒)",
        "Б) Неопределенность и риски. (😰)",
        "В) Рутина и отсутствие смысла. (😔)",
        "Г) Необходимость сдерживать эмоции. (🌋)",
        "Д) Монотонность без быстрых побед. (⚡)",
        "Е) Конфликты и негативная атмосфера. (🎭)",
        "Ж) Нет времени на восстановление. (☯)"
    ]
    random.shuffle(options)
    keyboard = [[KeyboardButton(text=option)] for option in options]
    return ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True, one_time_keyboard=True)
...
def get_question_5_keyboard() -> ReplyKeyboardMarkup:
    ...
    options = [
        "А) Навожу порядок, планирую. (🔒)",
        "Б) Ухожу в спокойную рутину. (😰)",
        "В) Уединяюсь в тишине. (😔)",
        "Г) Выплескиваю эмоции через спорт или музыку. (🌋)",
        "Д) Меняю обстановку, ищу новое. (⚡)",
        "Е) Провожу время с близкими. (🎭)",
        "Ж) Сон, еда, хобби, баланс. (☯)"
    ]
    random.shuffle(options)
    keyboard = [[KeyboardButton(text=option)] for option in options]
    return ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True, one_time_keyboard=True)
